...
job_options = {f"{job['title']} @ {job['company_name']}": job["id"] for job in jobs}
...
if desired_job_id:
    if students_data:  # If students data exists and is not empty
        for student in students_data:
            StudentCard(student)
    else:  # If students_data is empty
        ui.element("h3", children=["No students available for this position"], className="text-xl font-bold text-gray-800")
else:
    ui.element("h3", children=["Please select a valid job position to see the associated students."], className="text-xl font-bold text-gray-800")
