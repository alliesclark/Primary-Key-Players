...
with ui.element("div", className="flex flex-row justify-center items-center", key="winston_buttons"):
    ...
    with ui.element("div", className="flex flex-col m-2", key="damian_button_3"):
        if ui.button(text="See Job Postings from Similar Companies", key="3", className="bg-orange-300 text-white font-bold py-2 px-4 shadow rounded-lg w-full"):
            st.switch_page('pages/View_Similar_Jobs.py')
    ...
    with ui.element("div", className="flex flex-col m-2", key="damian_button_5"):
        if ui.button(text="Manage Job Applications", key="5", className="bg-orange-300 text-white font-bold py-2 px-4 shadow rounded-lg w-full"):
