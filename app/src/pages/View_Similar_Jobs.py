...
with ui.element("div", className="flex flex-col border rounded-lg shadow p-4 m-2", key="view_jobs_card"):
    ui.element("h2", children=["View Jobs from Similar Companies"], className="text-2xl font-bold text-gray-800", key="view_jobs_title")
    ui.element("div", children=["\n\n"], key="view_jobs_divider")
    ui.element("p", children=["Here you can view all the jobs posted on our platform that are in your industry. You can see the job title, description, location, desired skills, targeted majors, number of applicants, whether the job is still accepting applications, and the date the job was posted and updated."], className="text-gray-600")

data = {} 
try:
    data = requests.get('http://api:4000/j/job-position/similar-companies/1').json()
